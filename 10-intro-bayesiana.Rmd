...
**Definición.** (Probabilidad). Sea $E \subset \mathcal{E}$ un evento incierto
*relevante*, se define la probabilidad de $E$ como
$$\mathbb{P}(E) = \textsf{Area}(R)\,,$$
donde $R$ es un conjunto de referencia con el cual comparamos las decisiones
que son indiferentes (pues las consecuencias son equivalentes) a la realización 
del evento incierto. Esto lo denotamos por $d_R = d_E$.
...
- Si $E \cap F = \emptyset$ entonces $\mathbb{P}(E \cup F) = \mathbb{P}(E) + \mathbb{P}(F)$.
...
$$ \mathbb{P}(X = x \,|\, \theta) = {2 \choose x} \theta^x (1- \theta)^{2-x}\,.$$
...
$$\mathbb{P}(\theta=0.4 \,|\, X=2)\,.$$

¿Cómo calculamos esta probabilidad? ¿Qué sentido tiene?

Usando reglas de probabilidad condicional, podríamos calcular
$$\mathbb{P}(\theta=0.4|X=2) = \frac{\mathbb{P}(X=2 | \theta = 0.4) \mathbb{P}(\theta =0.4)}{\mathbb{P}(X=2)}\,.$$

Nota que en el numerador uno de los factores, $\mathbb{P}(X=2 | \theta = 0.4),$ es la
verosimilitud. Así que primero necesitamos evaluarla:
$$\mathbb{P}(X=2|\theta = 0.4) = (0.4)^2 = 0.16\,.$$

La novedad es que ahora tenemos que considerar la probabilidad
$\mathbb{P}(\theta = 0.4)$. Esta cantidad no la habíamos encontrado
...
moneda entre lanzamientos. Entonces con $\mathbb{P}(\theta)$ lo que estamos codificando
...
\begin{align*}
\mathbb{P}(\text{tener la moneda cargada a águila}) &= \mathbb{P}(\theta = 0.4) \\
&= \mathbb{P}(\theta = 0.6)\\
&= \mathbb{P}(\text{tener la moneda cargada a sol}) \\
&= 0.5\,,
\end{align*}
que representa la probabilidad de escoger de manera aleatoria la moneda con
una carga en particular.

Ahora, lo que nos falta es calcular $\mathbb{P}(X=2)$, pero con el trabajo que
...
\begin{align}
\mathbb{P}(X) &= \sum_{\theta \in \Theta} \mathbb{P}(X, \theta)\\
&= \sum_{\theta \in \Theta} \mathbb{P}(X\, |\, \theta) \mathbb{P}(\theta),
\end{align}
lo cual en nuestro ejemplo se traduce en escribir

$$ \mathbb{P}(X=2) = \mathbb{P}(X=2|\theta = 0.4)\mathbb{P}(\theta = 0.4) + \mathbb{P}(X=2|\theta=0.6)\mathbb{P}(\theta =0.6)\,,$$
por lo que obtenemos
$$\mathbb{P}(X=2) = 0.16(0.5) + 0.36(0.5) = 0.26\,.$$

```{block, type = "comentario"}

La función de probabilidad $\mathbb{P}(X)$ tiene un lugar muy especial en inferencia
bayesiana y dependiendo el uso recibe dos nombres.

- Si la evaluamos en nuestros datos $\mathbb{P}(X = 2)$ se conoce como la **evidencia**
de nuestros datos.
- Si la evaluamos en un caso hipótetico $\mathbb{P}(X = x^*)$ se conoce como la **predictiva previa**.
...
$$\mathbb{P}(\theta=0.4|X=2) = \frac{0.16(0.5)}{0.26} \approx  0.31\,.$$
...
Sea $\mathbb{P}(E_{i,j}| d_i)$ la probabilidad de que suceda $E_{i,j}$ si se elige
...
$$\mathbb{E}[u(d_i)] = \sum_{j=1}^{m_i} u(c_{i,j}) \mathbb{P}(E_{i,j}| d_i)\,.$$
...
\begin{align}
\mathbb{P}(E_i |Z) = \frac{\mathbb{P}(Z|E_i)\mathbb{P}(E_i)}{\sum_{j \in J}\mathbb{P}(Z|E_j)\mathbb{P}(E_j)}\,.
\end{align}
...
interesaría saber:
$$\mathbb{P}({X}_{nv}=\mathsf{sol}\, | \, X=2)\,.$$

Notemos que un volado adicional es un resultado binario. Como tenemos las
probabilidades posteriores $\mathbb{P}(\theta|X=2)$, podemos usar probabilidad total
condicionado en $X=2$:
\begin{align*}
\mathbb{P}({X}_{nv}=\mathsf{sol}\, | \, X=2) & = \sum_{\theta \in \Theta} \mathbb{P}({X}_{nv}=\mathsf{sol}, \theta \, | \, X=2) & \text{(probabilidad total)}\\
&= \sum_{\theta \in \Theta} \mathbb{P}({X}_{nv}=\mathsf{sol}\, | \theta , X=2) \mathbb{P}(\theta \, | \, X=2) & \text{(probabilidad condicional)}\\
&= \sum_{\theta \in \Theta} \mathbb{P}({X}_{nv}=\mathsf{sol}\, | \theta ) \mathbb{P}(\theta \, | \, X=2)\,, & \text{(independencia condicional)}
\end{align*}

lo que nos da el siguiente cálculo
$$\mathbb{P}(X_{nv}=\mathsf{sol}\, |\, \theta=0.4) \,  \mathbb{P}(\theta=0.4|X=2) \,  +\, \mathbb{P}(X_{nv}=\mathsf{sol}|\theta = 0.6) \, \mathbb{P}(\theta =0.6|X=2)\,.$$
Es decir, promediamos ponderando con las probabilidades posteriores.
Por lo tanto obtenemos
$$\mathbb{P}(X_{nv} = \mathsf{sol}|X=2) =  0.4 ( 0.31) + 0.6 (0.69) = 0.538\,.$$

```{block, type = "comentario"}

La función de probabilidad $\mathbb{P}(X_{nv} | X)$ recibe un nombre especial en
...
proceso. Es decir, que no tenemos información $X \sim \mathbb{P}(X|\theta)$ entonces
nuestro estado de conocimiento marginal, $\mathbb{P}(X)$, se conoce como la
**distribución predictiva previa** y se obtiene
$$ \mathbb{P}(X) = \sum_{\theta \in \Theta}^{} \mathbb{P}(X | \theta) \mathbb{P}(\theta)\,.$$

Cuando hemos recabado información, utilizamos probabilidad condicional para
determinar qué valores nuevos $X^*$ son los mas probables una vez que hemos observado mediciones. Esto es $\mathbb{P}(X^*|X)$ y se obtiene al marginalizar utilizando
nuestro estado actual de conocimiento. Es decir,
$$ \mathbb{P}(X^*|X) = \sum_{\theta \in \Theta}^{} \mathbb{P}(X^* | \theta) \mathbb{P}(\theta|X)\,.$$
...
que denotamos por $\mathbb{P}(\theta)$ (o respectivamente $\mathbb{P}(\theta | \textsf{datos})$).
...
- Como $X$ son los datos observados, llamamos a $\mathbb{P}(X|\theta)$ la *verosimilitud*,
proceso generador de datos o modelo de los datos.
- El factor $\mathbb{P}(\theta)$ le llamamos la distribución *inicial* o *previa*.
- La distribución que usamos para hacer inferencia $\mathbb{P}(\theta|X)$ es la
distribución *final* o *posterior*

Hacemos inferencia usando la ecuación

$$\mathbb{P}(\theta | X) = \frac{\mathbb{P}(X | \theta) \mathbb{P}(\theta)}{\mathbb{P}(X)} = \frac{\mathbb{P}(X | \theta) \mathbb{P}(\theta)}{\sum_{\theta \in \Theta} \mathbb{P}(X | \theta) \mathbb{P}(\theta)}\,,$$

que también escribimos:

$$\mathbb{P}(\theta | X) \propto \mathbb{P}(X | \theta) \mathbb{P}(\theta)\,,$$
donde $\propto$ significa "proporcional a". No ponemos $\mathbb{P}(X)$ pues como vimos
arriba, es una constante de normalización (y en la mayoría de los casos es muy dificil poder derivar analiticamente).

```


En estadística Bayesiana, las probablidades posteriores $\mathbb{P}(\theta|X)$ dan toda
...
$$\mathbb{P}(X_1=x_1,X_2 =x_2,\ldots, X_n=x_n|\theta) = {n \choose k} \theta^k(1-\theta)^{n-k}\,,$$

donde $k = x_1 + x_2 +\cdots + x_k$ es el número de éxitos que observamos.

Ahora necesitamos una distribución inicial o previa $\mathbb{P}(\theta)$. Aunque esta
...
$$\mathbb{P}(\theta) = \frac{1}{B(3,3)} \theta^{3-1}(1-\theta)^{3-1}\,,$$
...
$$\mathbb{P}(\theta| X_1=x_1, \ldots, X_n=x_n) \propto \mathbb{P}(X_1 = x_1,\ldots
X_n=x_n | \theta)\mathbb{P}(\theta) = \theta^{k+2}(1-\theta)^{n-k + 2}\,,$$
...
$$\mathbb{P}(\theta | S_n = 19) \propto \theta^{19 + 2} (1-\theta)^{30 - 19 +2} = \theta^{21}(1-\theta)^{13}\,.$$

La cantidad de la derecha, una vez que normalizemos por el número $\mathbb{P}(X=19)$, nos
...
$$\mathbb{P}(X = k) = \int_0^1 \mathbb{P}(X = k|\theta) \mathbb{P}(\theta) d\theta\,,$$
lo cual cual nos lleva a
\begin{align}
\mathbb{P}(X = k) &= \int_{0}^{1} {n \choose k} \theta^{k}(1- \theta)^{n-k} \frac{1}{B(a,b)} \theta^{a-1} (1-\theta)^{b-1} d\theta\\
...
$$\mathbb{P}(X_1,\ldots, X_n|\theta) = \theta^{-n} \times 1_{[M_n,\infty)}(\theta)\,,$$
cuando $\theta$ es mayor que todas las $X_i$ (o bien, cuando $\theta$ es mayor
que el máximo de las observaciones $M_n = \max(X_1, \ldots, X_n)$), y cero en
otro caso. Necesitaremos una inicial $P(\theta)$.

Por la forma que tiene la verosimilitud, podemos intentar una distribución Pareto,
que tiene la expresión
$$\mathbb{P}(\theta) = \frac{\alpha \theta_0^\alpha}{\theta^{\alpha + 1}} 1_{[\theta_0,\infty]}(\theta) \,,$$
...
$$\mathbb{P}(\theta|X_1,\ldots, X_n |\theta) \propto \theta^{-(n + 2.1)} \left(1_{[M_n,\infty)}(\theta)  \times 1_{[\theta_0,\infty]}(\theta)\right) \,,$$
...
Bajo un modelo conjugado, la distribución posterior $\mathbb{P}(\theta | X_1, \ldots, X_n)$
satisface
$$\mathbb{P}(\theta \, | \, X_1, \ldots, X_n) = \pi(\theta \, | \, \varphi_1)\,,$$
...
como distribuciones iniciales (dadas por 4 parámetros: $\mu_0, n_0, \alpha,\beta$):
  ...
  - $\mathbb{P}(\mu, \sigma) = \mathbb{P}(\mu|\sigma)\mathbb{P}(\sigma)$
...
- Entonces la posterior es:
  ...
  - $\mathbb{P}(\mu,\sigma|x) = \mathbb{P}(\mu|x,\sigma)\mathbb{P}(\sigma|x)$
...
$\mathbb{P}(\theta_1,\theta_2,\cdots, \theta_p|x)$. Este último ejemplo es relativamente
...
Pusimos banda de referencia entre alturas de 150 y 180. Vemos que nuestras
iniciales no producen simulaciones totalmente fuera del contexto, y parecen
cubrir apropiadamente el espacio de posiblidades para estaturas de los tenores.
Quizá hay algunas realizaciones poco creíbles, pero no extremadamente. En este
punto, podemos regresar y ajustar la inicial para $\sigma$, que parece tomar
valores demasiado grandes (produciendo por ejemplo una simulación con estatura
de 220 y 140, que deberían ser menos probables).
...
ggplot(sims %>% filter(dist == "posterior"),
  ...
  coord_equal()
```
Podemos construir intervalos de credibilidad del 90% para estos dos parámetros,
por ejemplo haciendo intervalos de percentiles:
...
Ahora solo resta checar que el modelo es razonable. Veremos más adelante cómo
hacer esto, usando la distribución predictiva posterior.
...
Supongamos que tenemos la posterior $\mathbb{P}(\theta | x)$. Podemos generar una nueva
*replicación* de los datos como sigue:

```{block, type='mathblock'}
La distribución **predictiva posterior** genera nuevas observaciones a partir de
la información observada. La denotamos como $\mathbb{P}(\tilde{x}|x)$.

Para simular de ella:

- Muestreamos un valor $\tilde{\theta}$ de la posterior $\mathbb{P}(\theta|x)$.
- Simulamos del modelo de las observaciones $\tilde{x} \sim \mathbb{P}(\tilde{x}|\tilde{\theta})$.
...
- Muestreamos un valor $\tilde{\theta}$ de la posterior $\mathbb{P}(\theta|x)$.
- Simulamos del modelo de las observaciones $\tilde{x}_1, \tilde{x}_2,\ldots, \tilde{x}_n \sim \mathbb{P}(\tilde{x}|\tilde{\theta})$,
...
Vamos a crear varias muestras, del mismo tamaño que la original, según nuestra
predictiva posterior, y compararemos estas muestras con la observada.
...
$$\mathbb{P}(\lambda|x_1,\ldots, x_n) \propto e^{-n\lambda}\lambda^{\sum_i x_i} e^{-0.1\lambda} = \lambda^{n\overline{x}}e^{-\lambda(n + 0.1)}\,,$$
...
Y con esto obtenemos el intervalo (163, 189), al 95%, para una nueva
observación. Nótese que este intervalo no puede construirse con una simulación
particular de la posterior de los parámetros, pues sería demasiado corto.

Es posible demostrar que en este caso, la posterior predictiva tiene una forma
conocida:

- La posterior predictiva para el modelo normal-gamma inverso es una
distribución $t$ con $2\alpha'$ grados de libertad, centrada en $\mu'$, y con
escala $s^2 = \frac{\beta'}{\alpha'}\frac{n + n_0 + 1}{n +n_0}$
...
Hasta ahora hemos argumentado que el tipo de resultado de
un análisis de datos bayesiano considera la distribución posterior completa. Se
puede resumir esta distribución y reportar ciertas cantidades
puntuales (para realizar estimación puntual) o se pueden reportar
intervalos de probabilidad (para realizar estimación por intervalos).
Ambas ideas también las hemos explorado bajo un enfoque frecuentista.

Para **estimación puntual** consideremos que $\mathcal{D} = \mathcal{E} =
\Theta$.  Para la función de utilidad consideraremos $v(\hat \theta,
\theta)$ la pérdida de estimar $\hat \theta$ el verdadero parámetro de
interés $\theta$. Ahora consideraremos casos puntuales:

1. **Pérdida cuadrática**:
$$v(\hat \theta,\theta) = (\hat \theta- \theta)^2\,.$$

En este caso la decisión óptima que minimiza la pérdida esperada es
$$\hat \theta = \mathbb{E}(\theta |X)\,.$$
Es decir, la media de la distribución que contempla nuestro estado de
conocimiento.

2. **Pérdida en valor absoluto**:
$$v(\hat \theta,\theta) = |\hat \theta- \theta|\,.$$

En este caso la decisión óptima que minimiza la pérdida esperada es
$$\hat \theta = \mathsf{mediana}(\theta | X)\,.$$
Es decir, la mediana de la distribución que contempla nuestro estado de
conocimiento.

3. **Pérdida vecindad**:
$$v(\hat \theta,\theta) = 1 - \mathbb{I}_{B_\epsilon(\hat \theta)}(\theta)\,,$$
denota la vecindad con radio $\epsilon$ alrededor de $\hat \theta$.
En este caso, la decisión óptima que minimiza la pérdida esperada
cuando $\epsilon \rightarrow 0$ es

$$\hat \theta = \mathsf{moda}(\theta|X)\,.$$
Es decir, la moda media de la distribución que contempla nuestro estado de
conocimiento.

Para **estimación por intervalos** consideramos que $\mathcal{D} = \{
I : I \subset \Theta\}$, donde $I$ es un intervalo de probabilidad
$(1-\alpha)$ si
$$ \int_{I}^{} \mathbb{P}(\theta | X)d\theta = 1 - \alpha\,.$$

**Nota** que para una $\alpha$ fija no existe un intervalo único de
probabilidad.

El espacio de eventos es el espacio parametral y la función de pérdida es
$$v(I, \theta) = \|D\| - 1_I(\theta)\,.$$
De lo anterior, observamos que para un nivel dado $\alpha$ es preferible
reportar el intervalo de probabilidad que sea de tamaño mínimo.

Para **contraste de hipótesis** consideremos por simplicidad, dos hipótesis que
denotaremos por $H_0$ y $H_1$. Estas pueden representar dos modelos
distintos. En este caso (en el marco de decisión) el espacio de
alternativas está definido por
$$ \mathcal{D} = \{H_0, H_1\}\,.$$
El espacio parametral (espacio de eventos) está definido en términos
de estas posibilidades. Es decir,
$$ \mathcal{E}= \{H_0, H_1\}\,.$$
La función de pérdida la definimos en términos de la decisión que tomamos
y lo que en realidad sucede. Esta función de pérdida la representamos como
...
donde $v_{0,0}$, $v_{1,1}$ son la pérdida de tomar una decisión correcta
(usualmente igual a 0). El costo $v_{1,0}$ es la pérdida de rechazar $H_0$
cuando $H_0$ es cierta y $v_{0,1}$ es la pérdida de no rechazar $H_0$ cuando
$H_0$ es falsa.

Se denotamos por $p_0 = \mathbb{P}(H_0)$ la probabildiad asociada a la hipótesis
$H_0$ al momento de tomar una decisión, la cual refleja nuestro estado de
conocimiento (previo o posterior). Entonces la pérdida esperada para cada
decisión es
\begin{gather}
\mathbb{E}[v(H_0)] = v_{0,0} p_0 + v_{0,1} (1 - p_0) = v_{0,1} - (v_{0,1}  - v_{0,0} ) p_0\,, \\
\mathbb{E}[v(H_1)] = v_{1,0} p_0 + v_{1,1} (1 - p_0) = v_{1,1} - (v_{1,1}  - v_{1,0} ) p_0\,.
\end{gather}

Que podemos pensar son dos rectas que intersectan en el punto
\begin{align}
p^* = \frac{v_{0,1} - v_{1,1}}{v_{1,0}- v_{1,1} + v_{0,1} - v_{0,0}} \,.
\end{align}

De donde se sigue que si
\begin{align}
\mathbb{E}[v(H_0)] < \mathbb{E}[v(H_1)] \Leftrightarrow \frac{p_0}{1 - p_0} < \frac{ v_{0,1} - v_{0,0}}{v_{1,0}- v_{1,1}} \Leftrightarrow p_0 > p^*\,,
\end{align}
que implica escoger $H_0$. Lo cual indica que si $p_0$ es
*suficientemente* grande comparada con $1 - p_0$ entonces decidiríamos
por $H_0$.

El caso contrario es análogo y podemos ver si $p_0$ es *suficientemente*
pequeña comparada con $1-p_0$ entonces decidimos por $H_1$.

Desde el punto de vista bayesiano tenemos una tercera opción que no teníamos bajo
el enfoque frecuentista de contraste de hipótesis. Si $p_0 = p^*$ entonces somos
indiferentes y no podemos decir por una u otra alternativa.

### Incorporando observaciones para contraste hipótesis {-}

Siguiendo con el marco de contraste de hipótesis bayesiana podemos considerar que
en las ecuaciones de arriba sólo hemos utilizado $\mathbb{P}(H_0)$. Es decir,
la probabilidad a priori de que ocurra $H_0$. Sin embargo, siguiendo un poco la
intuición de nuestra discusión con las función predictiva, es natural considerar
la distribución posterior de $H_0$ una vez observados los datos $\mathbb{P}(H_0|X)$.

Suponiendo que los costos de no equivocarnos son 0, entonces escogeríamos $H_0$ si
y sólo si
\begin{align}
\frac{\mathbb{P}(H_0|X)}{\mathbb{P}(H_1|X)} < \frac{v_{0,1}}{v_{1,0}}\,.
\end{align}
Lo cual es equivalente a
\begin{align}
\frac{\mathbb{P}(X|H_0)}{\mathbb{P}(X|H_1)} < \frac{\mathbb{P}(H_1)}{\mathbb{P}(H_0)} \times \frac{v_{0,1}}{v_{1,0}}\,,
\end{align}

que nos indica que la decisión (o indecisión) se toma de acuerdo al
contraste del ajuste de un modelo sobre otro (cociente de
verosimilitudes) contra un factor que incorpora un balance entre los
costos asociados a la decisión incorrecta y los pesos relativos de
