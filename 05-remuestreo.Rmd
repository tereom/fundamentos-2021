...
```{r, cache = TRUE, fig.height = 2.5, out.width="99%"}
...
```{r, echo = FALSE, message = FALSE, cache = TRUE, fig.height = 2.5, out.width="99%"}
set.seed(911)
...
```{r, echo = FALSE, fig.height = 2.5, out.width="99%"}
...
```{r, message = FALSE, warning = FALSE, fig.height=3, out.width="99%"}
...
tab_out <- tabla_perfil %>% left_join(precio_prom) %>%
  ...
  knitr::kable(format = format_table_salida(), escape = F, digits = 2) %>%
...
```{r, out.width = "99%",echo = FALSE, fig.cap= "Extracto de estimación de errores de muestreo, ENIGH 2018."}
...
$$\hat{\textsf{se}}_B = \bigg\{\frac1B \sum_{b=1}^B[\hat{\theta}^*(b)-\hat{\theta}^*(\cdot)]^2 \bigg\}^{1/2}$$
