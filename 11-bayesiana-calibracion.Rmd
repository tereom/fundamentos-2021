...
library(tidyverse)
library(patchwork)
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning=FALSE, fig.align = 'center', fig.width = 6, fig.height=3)
comma <- function(x) format(x, digits = 2, big.mark = ",")
theme_set(theme_minimal())
```

El enfoque bayesiano se puede formalizar coherentemente en términos
de probabilidades subjetivas, y como vimos, esta es una fortaleza.

En la práctica, sin embargo, muchas veces puede ser difícil argumentar en
términos exclusivos de probabilidad subjetiva, aunque hagamos los esfuerzos
apropiados para incorporar la totalidad de información que distintos actores
involucrados pueden tener.

Consideremos, por ejemplo, que INEGI produjera un intervalo creíble del 95% para
el ingreso mediano de los hogares de México. Aún cuando nuestra metodología sea
transparente y correctamente informada, algunos investigadores interesados puede
ser que tengan recelo en usar esta información, y quizá preferirían hacer
estimaciones propias. Esto restaría valor al trabajo cuidadoso que pusimos en
nuestras estimaciones oficiales.

Por otra parte, el enfoque frecuentista provee de ciertas garantías mínimas para
la utilización de las estimaciones, que no dependen de la interpretación
subjetiva de la probabilidad, sino de las propiedades del muestreo. Consideremos
...
**Observación.** Checar que la cobertura real es similar a la nominal es
importante en los dos enfoques: frecuentista y bayesiano. Los intervalos
frecuentistas, como hemos visto, generalmente son aproximados, y por lo tanto no
cumplen automáticamente esta propiedad de calibración.
...
la distribución inicial $\theta \sim \mathsf{Beta}(2,2)$, y la verosimilitud estándar
binomial, vimos que la posterior cuando observamos $k$
éxitos es  $$\theta|k \sim \mathsf{Beta}(k + 2, n - k + 2)\,.$$
...
Este intervalo lo compararemos con el usual de Wald: si $\hat{\theta} = \frac{k}{n}$, entonces
...
parametros <- crossing(n = c(5, 10, 30, 60, 100, 400),
                       p = c(0.01, 0.015, 0.02, 0.025, 0.03,
                             0.035, 0.04, 0.05, 0.07, 0.1, 0.15))
set.seed(2343)
# simulaciones
...
**Conclusión 1**: Los intervalos de Agresti-Coull son una buena alternativa para
estimar proporciones como sustituto de los intervalos clásicos de Wald, aunque
tienden a ser muy conservadores para muestras chicas.

Idealmente podemos utilizar un método bayesiano pues normalmente tenemos
información inicial acerca de las proporciones que queremos estimar.

## Incorporando información inicial {-}

Nótese que generalmente tenemos información acerca de la cantidad que queremos
estimar: por ejemplo, que proporción de visitantes de un sitio web compra algo
(usualmente muy baja, menos de $2\%$), qué proporción de personas tiene diabetes
tipo 1 (una proporción muy baja, menos de 1 por millar), o qué proporción de
hogares tienen ingresos trimestrales mayores a 150 mil pesos (menos de $5\%$ con
alta probabilidad).
...
la longitud del intervalo bayesiano es **mucho menor** que el de
...
Cuando la muestra es chica, los intervalos bayesianos son similares a los
iniciales, y mucho más cortos que los de Agresti-Coull. Para muestras
intermedias (50-100) los intervalos bayesianos son más informativos que los de
Agresti-Coull, con calibración similar, y representan aprendizaje por encima de
lo que sabíamos en la inicial. Para muestras grandes, obtenemos resultados
simililares.
...
```
es un intervalo muy grande que puede incluir valores negativos. En contraste, el
intervalo bayesiano es:
...
Usaremos los datos de ENIGH como ejemplo (ignorando el diseño, pero es posible
hacer todas las estimaciones correctamente) para estimar el porcentaje de
hogares que tienen ingreso corriente de más de 150 mil pesos al trimestre.
Suponemos que la muestra del ENIGH es la población, y tomaremos una muestra iid
de esta población. Usamos la misma inicial que mostramos arriba, que es una Beta
con parámetros
...
estimadores_sim_ag <- estimadores_sim %>%
  ...
  summarise(n = n())
ggplot(estimadores_sim_ag, aes(x = est_mv, media_post, size = n)) +
  geom_point() +
  geom_abline()
```
...
y el verdadero valor $\theta$ poblacional. Es posible considerar distintas
funciones de pérdida, pero como en secciones anteriores (0-1, absoluta,
norma-$p$, etc.), usaremos la **pérdida
cuadrática**, la cual está definida por:

$$L(\theta, \hat{\theta}_n) = (\theta - \hat{\theta}_n)^2\,,$$

Esta función toma distintos valores dependiendo de la muestra (reflejada en
nuestro estimador) y del parámetro $\theta$, y necesitamos resumirla para dar
una evaluación de qué tan bueno es el estimador $\hat{\theta}_n.$
...
donde promediamos con respecto a conjuntos de muestras posibles de tamaño $n$,
con $\theta$ fijo. Esta cantidad no nos dice necesariamente cómo escoger un buen
estimador para $\theta$, pues dependiendo de dónde está $\theta$ la función de
riesgo puede tomar valores distintos. La figura siguiente ilustra esto: 


```{r, echo = FALSE, fig.cap = "Riesgo frecuentista para distintos estimadores."}
tibble(t = seq(-1,1, by = .01), 
       risk1 = 20 * (.5 - t)**2 - .5, 
       risk2 = (t)**2, 
       risk3 = (t)**2 + .5) |> 
  pivot_longer(cols = 2:4, names_to = "curva") |> 
  mutate(curva = factor(curva, labels = c("Riesgo 1", "Riesgo 2", "Riesgo 3"))) |> 
  ggplot(aes(x = t, y = value)) + 
    geom_line(aes(group = curva, color = curva)) + 
    ylim(-.5, 2) + xlab("Párametro")  + ylab("Riesgo") + 
    scale_color_manual(values = c('Riesgo 1' = 'red', 'Riesgo 2' = 'blue', "Riesgo 3" = "green"), 
                       labels = expression(R(theta, t[1]), R(theta, t[2]), R(theta, t[3])))

```

donde $t_j$ son posibles estimadores de $\theta$ en este problema ficticio. Nota
que no es claro que estimador domina a otro. ¿Cómo podemos escoger entre el
estimador $t_1$ y $t_2$? ¿Cómo consideramos bajo un argumento frecuentista 
que $\theta$ es un valor que no conocemos y que afecta la selección del
procedimiento (estimador) adecuado de acuerdo a nuestro criterio (función de
pérdida)?

La respuesta es bajo un enfoque frecuentista podemos tomar cualquiera de las 
tres alternativas:   
- Admisibilidad. Una decision inadmisible es aquella que es dominada por otra en
cualquier valor de $\theta$.  
- Restringir la clase de estimadores. Por ejemplo, considerar sólo estimadores
insesgados.  
- Minimax. Nos fijamos en aquel procedimiento bajo $\max_\Theta R(\theta, t)$.  


La dependencia en $\theta$ (el valor del parámetro que no conocemos) no nos
permite tomar una decisión tan clara bajo $R(\theta, t)$. 

**Ejemplo.** Considera una muestra $X\sim \mathsf{N}(\theta, 1)$. Asume que
estamos utilizando error cuadrático como función de pérdida y considera los
estimadores: 1) $\hat \theta = X$ y 2) $\hat \theta = 3$. Bajo estos estimadores 
tenemos las siguientes funciones de riesgo:  
1) $R(\theta, \hat \theta_1)= 1$.  
2) $R(\theta, \hat \theta_2)= (3-\theta)^2$.   

¡No podemos decir cuál es mejor estimador independientemente del valor de $\theta$!
...
$\theta$ es una cantidad aleatoria con distribución inicial $\mathbb{P}(\theta)$.
Entonces consideraríamos el promedio sobre la posterior dado por:

$$\rho(X, t) = \mathbb{E}\left[(\theta - \hat{\theta})^2 \, \big | \, X \right]\,,$$
...
estimador dados los datos. ¡Esto ya lo hemos estudiado en la sección anterior!

En el primer caso, promediamos sobre posibles muestras, y en el segundo por
valores posibles de $\theta$ para una muestra dada (ponderada con nuestro estado
de conocimiento actual).

### Ejemplo: riesgo frecuentista {-}

Para observaciones $\textsf{Bernoulli}$, el estimador de máxima verosimilitud es
$\hat{p}_1 = k /n$, donde $n$ es el tamaño de muestra y $k$ el número de éxitos
observados. Podemos usar también como estimador la media posterior de un modelo
...
```{r, fig.height=2.5}
p_seq <- seq(0, 1, 0.001)
riesgo_tbl <- map(p_seq, ~ estimar_riesgo(n = 20,
                                          theta = .x,
                                          perdida = perdida_cuad)) %>%
  bind_rows()
...
Ahora considera el caso de una Beta con parámetros $a = b = \sqrt{n/4}$, 
que nos daría $\hat{p}_2 = \frac{k + \sqrt{n/4}}{n + \sqrt{n}}$.

```{r}
t_3 <- function(n, x)  (x + sqrt(n/4)) / (n + sqrt(n))
estimar_riesgo <- function(n = 20, theta, perdida, reps = 10000){
  x <- rbinom(reps, n, theta)
  # calcular estimaciones
  theta_1 <- t_1(n, x)
  ...
  media_perdida <- tibble(
    n = n, theta = theta,
    estimador = c("MLE", "Posterior", "Posterior 2"),
    riesgo = c(mean(perdida(theta, theta_1)), 
               mean(perdida(theta, theta_2)),
               mean(perdida(theta, theta_3))
               ))
  media_perdida
}
```
...
Nota cómo este tercer estimador tiene un riesgo constante (independiente de
$\theta$). Sin embargo, con un estimador con riesgo constante no podríamos
identificar cuál es el mejor estimador. Es decir, no podemos decir qué estimador
tiene un riesgo menor para cualquier valor de $\theta$.

### Ejemplo: riesgo posterior {-}

Ahora, supongamos que tenemos un conjunto de datos fijo. Y consideremos
distintas realizaciones bajo la distribución posterior utilizando una inicial
$\theta \sim \mathsf{Beta}(5,3)$:

```{r}
estimar_riesgo_post <- function(n = 20, x, perdida, reps = 50000){
  # calcular estimaciones
  theta_1 <- t_1(n, x)
  theta_2 <- t_2(n, x)
  theta_3 <- t_3(n, x)
  ...
  media_perdida <- tibble(
    n = n, x = x,
    estimador = c("MLE", "Posterior", "Posterior 2"),
    riesgo_post= c(mean(perdida(theta_post, theta_1)),
                   mean(perdida(theta_post, theta_2)), 
                   mean(perdida(theta_post, theta_3))))
  media_perdida
}
estimar_riesgo_post(n = 20, x = 8, perdida = perdida_cuad)
```
Como dijimos, esta cantidad depende de un conjunto de datos $x$ fijo.
Consideremos que los datos, además, no los observamos **previo** a decidir qué
estimador utilizar. Así que calculamos para cada valor de $x$:

Las funciones de pérdida promedio $\rho(x, t_1)$ y $\rho(x, t_2)$ (dependen de
$x$) se ven aproximadamente como sigue:
...
Donde vemos que la pérdida del estimador bayesiano es mejor para valores
extremos de número de éxitos observado $x$, pero tiene más riesgo posterior para
valores chicos de $x$. En general es mejor el estimador $\theta_2$. El estimador
de máxima verosimilitud tiene más riesgo en los extremos. Esto es de esperarse
pues no tenemos la regularización que aporta la posterior. Igualmente, vemos más
riesgo para valores chicos de $x$ que para valores grandes. Esto es porque la
inicial está concentrada en valores reslativamente grandes de $\theta$.
...
el riesgo frecuentista con la inicial $\mathbb{P}(\theta)$
- Si no conocemos los datos observados, podemos promediar usando
datos generados por la marginal $\mathbb{P}(x)$ de $x$ bajo el modelo de datos
$\mathbb{P}(x|\theta)$ y la inicial $\mathbb{P}(\theta)$.

Por la ley de la esperanza iterada, estos dos resultados son iguales. La cantidad
resultante

$$r(t) = \int R(\theta,t) \mathbb{P}(\theta)\, d\theta = \int \left( \int (\theta - t)^2 \mathbb{P}(x|\theta) dx \right) \mathbb{P}(\theta)\, d\theta = \int \rho(X, t) \mathbb{P}(x) dx\,,$$
se llama **riesgo de Bayes** para el estimador $t$. La ultima igualdad nos
permite interpretar esto como una propiedad frecuentista para una
...
1. (Estimador de Bayes) Si tenemos los datos $X$, escogeríamos una función $t$
que minimice el riesgo posterior $\rho(X, t)$, y nuestro estimador es
$\hat{\theta}_n = t(X).$
2. (Regla de Bayes) Si no tenemos los datos, escogeríamos el estimador una
función $t$  que minimice el riesgo de Bayes $r(t)$, y estimaríamos usando
$\hat{\theta}_n = t(X).$
3. Pero como el riesgo de Bayes es el promedio del riesgo posterior, la solución
de 1 nos da la solución de 2. Es decir, el estimador que escogemos condicional a
los datos $X$ es el mismo que escogeríamos antes de escoger los datos, dada una
distribución inicial $\mathbb{P}(\theta).$
...
En nuestro ejemplo de arriba el máximo se alcanza en 0.5, y tomaríamos eso
evaluación de los estimadores $\theta_1$ o $\theta_2$.  Buscaríamos entonces
estimadores que minimicen este máximo, es decir, **estimadores minimax**. Pero
también es posible enfocar este problema considerando sólo estimadores
*insesgados*, lo que nos lleva por ejemplo a buscar estimadores con mínima
varianza. También podemos enfocarnos en buscar estimador *admisibles*, que son
aquellos cuyo riesgo no está dominado para toda $\theta$ por otro estimador, y
así sucesivamente.

Finalmente, es posible demostrar [ver @Wasserman] que típicamente, para muestras
grandes, el estimador de máxima verosimilitud  es cercano a ser minimax y además
es una regla de Bayes. Estas son buenas propiedades, pero debemos contar con que
el régimen asintótico se cumpla aproximadamente.
